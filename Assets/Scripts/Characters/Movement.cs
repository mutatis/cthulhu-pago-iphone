...
public class Movement : MonoBehaviour
{
    ...
    {
        Rigidbody2DUnidirectional,
        Rigidbody2DBidirectional,
        Rigidbody2DFourDirectional,
        Rigidbody2DEightDirectional
    }
    ...
    [SerializeField]
    public RigidBody2DUnidirectional movement2d;
}
...
/*
    void Awake()
    {
        ...
        if (rigidbody || rigidbody2D)
            usingPhysicEngine = true;

        if (collider)
            groundCheck = collider.bounds.min;

        if (rigidbody2D || collider2D)
            is3d = false;
    }
