...
public class FelipeText4 : MonoBehaviour {
	...
	void Start () 
	{
		...
		gh = PlayerPrefs.GetInt("Felipe");
		if(PlayerPrefs.GetInt("Felipe") > texts.Length)
		{
			PlayerPrefs.SetInt("Felipe", 0);
			gh = PlayerPrefs.GetInt("Felipe");
		}
		else
		{
			gh = PlayerPrefs.GetInt("Felipe");
			PlayerPrefs.SetInt("Felipe", (gh + 5));
		}
		gh = PlayerPrefs.GetInt("Felipe");
		text.text = texts[gh];
	}
